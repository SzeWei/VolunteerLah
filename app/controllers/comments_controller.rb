class CommentsController < ApplicationController
  ...
  def create
    ...
    @comment = current_user.comments.new(comment_params) if user_signed_in?
    respond_to do |format|
      if @comment.save
        format.html { redirect_to @event, notice: 'Comment was successfully created.' }
        format.json { render :show, status: :created, location: @comment }
      else
        format.html { redirect_to @event, notice: 'Failed to create comment.' }
  ...
  private
    ...
    def comment_params
      params.require(:comment).permit(:title, :text, :rating, :event_id)
    end
end
